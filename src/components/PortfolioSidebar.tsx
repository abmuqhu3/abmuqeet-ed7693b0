...
const PortfolioSidebar = ({ activeSection }: { activeSection: string }) => {
  ...
  return (
    <aside
      className={`hidden lg:flex flex-col h-full border-r border-border/30 bg-sidebar transition-all duration-300 ${
        collapsed ? "w-16" : "w-56"
      }`}
    >
      {/* Logo */}
        ...
        <div className="flex items-center gap-2">
          ...
          {!collapsed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="overflow-hidden">
              <span className="font-orbitron text-sm font-bold text-foreground tracking-wider">KAM</span>
              <p className="text-[9px] font-mono text-muted-foreground">systems.portfolio</p>
            </motion.div>
          )}
        </div>
      </div>
